...
lon3 = atan2(Y3,X3)*180/pi - 80.6077;
lon4 = atan2(Y4,X4)*180/pi - 80.6077;

lat1 = atan2(Z1, sqrt(X1.^2+Y1.^2))*180/pi;
lat2 = atan2(Z2, sqrt(X2.^2+Y2.^2))*180/pi;
lat3 = atan2(Z3, sqrt(X3.^2+Y3.^2))*180/pi;
lat4 = atan2(Z4, sqrt(X4.^2+Y4.^2))*180/pi;

lat = cat(1, lat1, lat2, lat3, lat4);
lon = cat(1, lon1, lon2, lon3, lon4);
